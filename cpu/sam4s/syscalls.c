 ...
 *
 * @file        syscalls.c
 * @brief       NewLib system calls implementations for SAM4S16C
 *
...
caddr_t heap_top = (caddr_t)&__sheap + 4;
...
caddr_t _sbrk_r(struct _reent *r, ptrdiff_t incr)
{
    ...
    if (((incr > 0) && ((heap_top + incr > &__eheap) || (heap_top + incr < res))) ||
        ((incr < 0) && ((heap_top + incr < &__sheap) || (heap_top + incr > res)))) {
        r->_errno = ENOMEM;
        res = (void *) -1;
    }
    else {
